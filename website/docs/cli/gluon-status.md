...
`gluon status` shows the execution status of the latest run or a specific execution.
...
```bash
gluon status --detailed
```

Show a specific execution:

```bash
gluon status --exec-id my-plan-20240601-a1b2c3
```
...
Use `gluon describe run <id>` for a fuller breakdown including metadata, timing, and job-level errors.
